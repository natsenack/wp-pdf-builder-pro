...
function pdf_builder_load_core()
{
    ...
    add_action('admin_enqueue_scripts', function($hook) {
        ...
        if ($hook === 'pdf-builder_page_pdf-builder-react-editor' || (isset($_GET['page']) && $_GET['page'] === 'pdf-builder-react-editor')) {
            ...
            $canvas_settings = [
                'default_canvas_format' => $settings['pdf_builder_canvas_format'] ?? 'A4',
                'default_canvas_orientation' => $settings['pdf_builder_canvas_orientation'] ?? 'portrait',
                'default_canvas_unit' => $settings['pdf_builder_canvas_unit'] ?? 'px',
                'default_canvas_dpi' => intval($settings['pdf_builder_canvas_dpi'] ?? 96),
                'default_canvas_width' => intval($settings['pdf_builder_canvas_width'] ?? 794),
                'default_canvas_height' => intval($settings['pdf_builder_canvas_height'] ?? 1123),
                ...
                'zoom_with_wheel' => ($settings['pdf_builder_canvas_zoom_with_wheel'] ?? '1') == '1',
                'pan_with_mouse' => ($settings['pdf_builder_canvas_pan_enabled'] ?? '1') == '1',
                'show_resize_handles' => ($settings['pdf_builder_canvas_show_resize_handles'] ?? '1') == '1',
                'handle_size' => intval($settings['pdf_builder_canvas_handle_size'] ?? 8),
                'handle_color' => $settings['pdf_builder_canvas_handle_color'] ?? '#007cba',
                'enable_rotation' => ($settings['pdf_builder_canvas_rotate_enabled'] ?? '1') == '1',
                'rotation_step' => intval($settings['pdf_builder_canvas_rotation_step'] ?? 15),
                'multi_select' => ($settings['pdf_builder_canvas_multi_select'] ?? '1') == '1',
                'copy_paste_enabled' => ($settings['pdf_builder_canvas_copy_paste_enabled'] ?? '1') == '1',
                ...
                'compress_images' => ($settings['pdf_builder_canvas_compress_images'] ?? '1') == '1',
                'image_quality' => intval($settings['pdf_builder_canvas_image_quality'] ?? 85),
                'max_image_size' => intval($settings['pdf_builder_canvas_max_image_size'] ?? 2048),
                'include_metadata' => ($settings['pdf_builder_canvas_include_metadata'] ?? '1') == '1',
                'pdf_author' => $settings['pdf_builder_canvas_pdf_author'] ?? 'PDF Builder Pro',
                'pdf_subject' => $settings['pdf_builder_canvas_pdf_subject'] ?? '',
                'auto_crop' => ($settings['pdf_builder_canvas_auto_crop'] ?? false) == '1',
                'embed_fonts' => ($settings['pdf_builder_canvas_embed_fonts'] ?? '1') == '1',
                'optimize_for_web' => ($settings['pdf_builder_canvas_optimize_for_web'] ?? '1') == '1',
                'enable_hardware_acceleration' => ($settings['pdf_builder_canvas_enable_hardware_acceleration'] ?? '1') == '1',
                'limit_fps' => ($settings['pdf_builder_canvas_limit_fps'] ?? '1') == '1',
                'max_fps' => intval($settings['pdf_builder_canvas_fps_target'] ?? 60),
                'auto_save_enabled' => ($settings['pdf_builder_canvas_auto_save'] ?? '1') == '1',
                'auto_save_interval' => intval($settings['pdf_builder_canvas_auto_save_interval'] ?? 5),
                'auto_save_versions' => intval($settings['pdf_builder_canvas_auto_save_versions'] ?? 10),
                'undo_levels' => intval($settings['pdf_builder_canvas_undo_levels'] ?? 50),
                'redo_levels' => intval($settings['pdf_builder_canvas_redo_levels'] ?? 50),
                'enable_keyboard_shortcuts' => ($settings['pdf_builder_canvas_keyboard_shortcuts'] ?? '1') == '1',
                'canvas_selection_mode' => $settings['pdf_builder_canvas_selection_mode'] ?? 'click',
                'debug_mode' => ($settings['pdf_builder_canvas_debug_mode'] ?? false) == '1',
                'show_fps' => ($settings['pdf_builder_canvas_show_fps'] ?? false) == '1'
            ];
