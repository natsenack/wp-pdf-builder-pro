  ...
  window.pdfBuilderReact = {
    initPDFBuilderReact: function(config) {
        ...
        return window.React.createElement('div', { style: { flex: 1, backgroundColor: '#d8d8d8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflow: 'auto' } },
          window.React.createElement('div', { 
            ...
            canvasElements.map((elem) => {
              ...
              if (elem.type === 'text') {
                content = window.React.createElement('div', { style: { overflow: 'hidden', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', whiteSpace: 'pre-wrap', wordBreak: 'break-word', padding: '4px', boxSizing: 'border-box', color: elem.color || '#000000', fontSize: elem.fontSize || '14px', fontWeight: elem.fontWeight || 'normal', fontStyle: elem.fontStyle || 'normal', textDecoration: elem.textDecoration || 'none', textAlign: elem.textAlign || 'left' } }, elem.text || 'Texte');
              } else if (elem.type === 'rectangle') {
                content = window.React.createElement('div', { style: { width: '100%', height: '100%', backgroundColor: bgColor } });
              } else if (elem.type === 'circle') {
                contentStyle = { borderRadius: '50%' };
                content = window.React.createElement('div', { style: { width: '100%', height: '100%', backgroundColor: bgColor, borderRadius: '50%' } });
              } else if (elem.type === 'line') {
                content = window.React.createElement('div', { style: { width: '100%', height: '100%', display: 'flex', alignItems: 'center' } }, window.React.createElement('div', { style: { width: '100%', height: '2px', backgroundColor: elem.borderColor || '#000000' } }));
              } else if (elem.type === 'image') {
                content = window.React.createElement('img', { src: elem.src || '', alt: 'Image', style: { width: '100%', height: '100%', objectFit: 'cover', backgroundColor: '#f0f0f0' }, onError: (e) => { e.target.style.display = 'none'; } });
              } else {
              ...
              return window.React.createElement('div', { key: elem.id, 
                ...
                style: Object.assign({ position: 'absolute', left: elem.x + 'px', top: elem.y + 'px', width: elem.w + 'px', height: elem.h + 'px', backgroundColor: bgColor, border: borderWidth + 'px solid ' + (isSelected ? '#dc3545' : borderColor), borderRadius: borderRadius + 'px', boxSizing: 'border-box', cursor: draggingElementId === elem.id ? 'grabbing' : 'grab', overflow: 'hidden', userSelect: 'none', transform: 'rotate(' + (elem.rotation || 0) + 'deg)', opacity: (elem.opacity || 100) / 100, transition: draggingElementId === elem.id ? 'none' : 'border 0.1s', zIndex: zIndex }, contentStyle)
              },
                content,
      ...
      const PropertiesPanel = function() {
        ...
        const currentTab = propertiesPanelTab[sel.id] || 'position';
        ...
        return window.React.createElement('div', { style: { width: '280px', backgroundColor: '#fff', borderLeft: '1px solid #e0e0e0', overflow: 'auto', display: 'flex', flexDirection: 'column' } },
          ...
          window.React.createElement('div', { style: { display: 'flex', gap: '2px', borderBottom: '2px solid #e0e0e0', backgroundColor: '#f9f9f9', padding: '4px' } },
            window.React.createElement('button', { onClick: () => setTab('position'), style: { flex: 1, padding: '6px', backgroundColor: currentTab === 'position' ? '#007bff' : '#f0f0f0', color: currentTab === 'position' ? '#fff' : '#333', border: 'none', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', borderRadius: '3px 3px 0 0' } }, '📐'),
            window.React.createElement('button', { onClick: () => setTab('appearance'), style: { flex: 1, padding: '6px', backgroundColor: currentTab === 'appearance' ? '#007bff' : '#f0f0f0', color: currentTab === 'appearance' ? '#fff' : '#333', border: 'none', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', borderRadius: '3px 3px 0 0' } }, '🎨'),
            window.React.createElement('button', { onClick: () => setTab('advanced'), style: { flex: 1, padding: '6px', backgroundColor: currentTab === 'advanced' ? '#007bff' : '#f0f0f0', color: currentTab === 'advanced' ? '#fff' : '#333', border: 'none', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold', borderRadius: '3px 3px 0 0' } }, '⚙️')
          ),
          window.React.createElement('div', { style: { flex: 1, overflow: 'auto', padding: '12px' } },
            currentTab === 'position' ? window.React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px' } },
              window.React.createElement('div', null,
                window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '3px', color: '#555' } }, 'Position X (px):'),
                window.React.createElement('input', { type: 'number', value: Math.round(sel.x), onChange: (e) => { sel.x = parseFloat(e.target.value) || 0; triggerRender('property-edit'); }, style: { width: '100%', padding: '5px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
              ),
              window.React.createElement('div', null,
              ...
              window.React.createElement('div', null,
                ...
                window.React.createElement('div', { style: { display: 'flex', gap: '6px', alignItems: 'center' } },
                  window.React.createElement('input', { type: 'range', min: '0', max: '100', value: sel.opacity || 100, onChange: (e) => { sel.opacity = parseInt(e.target.value) || 100; triggerRender('property-edit'); }, style: { flex: 1, cursor: 'pointer', height: '4px' } }),
                  window.React.createElement('span', { style: { fontSize: '11px', color: '#666', minWidth: '28px', textAlign: 'right' } }, (sel.opacity || 100) + '%')
                )
              ),
              window.React.createElement('div', null,
                window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '3px', color: '#555' } }, 'Fond:'),
                window.React.createElement('input', { type: 'color', value: sel.backgroundColor || '#ffffff', onChange: (e) => { sel.backgroundColor = e.target.value; triggerRender('property-edit'); }, style: { width: '100%', height: '32px', border: '1px solid #ccc', borderRadius: '3px', cursor: 'pointer', boxSizing: 'border-box' } })
              ),
              window.React.createElement('div', null,
                window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '3px', color: '#555' } }, 'Bordure:'),
                window.React.createElement('input', { type: 'color', value: sel.borderColor || '#000000', onChange: (e) => { sel.borderColor = e.target.value; triggerRender('property-edit'); }, style: { width: '100%', height: '32px', border: '1px solid #ccc', borderRadius: '3px', cursor: 'pointer', boxSizing: 'border-box' } })
              ),
              window.React.createElement('div', null,
                window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '3px', color: '#555' } }, 'Épaisseur (px):'),
                window.React.createElement('input', { type: 'number', value: sel.borderWidth || 1, onChange: (e) => { sel.borderWidth = Math.max(0, parseInt(e.target.value) || 1); triggerRender('property-edit'); }, style: { width: '100%', padding: '5px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' }, min: '0' })
              )
