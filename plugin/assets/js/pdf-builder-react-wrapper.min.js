    ...
    initPDFBuilderReact: function(config) {
        ...
        return window.React.createElement('div', { style: { flex: 1, backgroundColor: '#d8d8d8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflow: 'auto' } },
          window.React.createElement('div', { 
            ...
            onDrop: (e) => {
              ...
              const newElem = { id: 'elem_' + Date.now(), type: draggedElement.type, label: draggedElement.label, icon: draggedElement.icon, x: Math.max(0, e.clientX - rect.left - 50), y: Math.max(0, e.clientY - rect.top - 25), w: 100, h: 50, rotation: 0, opacity: 1 };
            ...
            canvasElements.map((elem) => {
              ...
              return window.React.createElement('div', { key: elem.id, 
                ...
                style: { position: 'absolute', left: elem.x + 'px', top: elem.y + 'px', width: elem.w + 'px', height: elem.h + 'px', backgroundColor: '#e8f4f8', border: (isSelected ? '2px solid #dc3545' : '2px solid #3b82f6'), borderRadius: '4px', padding: '6px', boxSizing: 'border-box', cursor: draggingElementId === elem.id ? 'grabbing' : 'grab', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: '#333', userSelect: 'none', fontSize: '10px', transform: 'rotate(' + elem.rotation + 'deg)', opacity: elem.opacity, transition: draggingElementId === elem.id ? 'none' : 'border 0.1s' }
              },
                window.React.createElement('div', null,
                  window.React.createElement('div', { style: { fontSize: '14px', marginBottom: '2px', lineHeight: '1' } }, elem.icon),
                  window.React.createElement('div', { style: { fontSize: '9px', color: '#666' } }, elem.label.substring(0, 12))
                ),
      ...
      const PropertiesPanel = function() {
        ...
        return window.React.createElement('div', { style: { width: '280px', backgroundColor: '#fff', borderLeft: '1px solid #e0e0e0', overflow: 'auto', display: 'flex', flexDirection: 'column' } },
          window.React.createElement('div', { style: { padding: '12px', borderBottom: '1px solid #e0e0e0', fontWeight: '600', fontSize: '13px', backgroundColor: '#f9f9f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
            window.React.createElement('span', null, ' Propriétés' + (sel ? ' (1)' : '')),
            sel && window.React.createElement('button', { onClick: () => { canvasElements = canvasElements.filter(e => e.id !== selectedElementId); selectedElementId = null; triggerRender('element-delete'); }, style: { padding: '4px 8px', border: '1px solid #dc3545', borderRadius: '3px', backgroundColor: '#dc3545', color: '#fff', cursor: 'pointer', fontSize: '11px', fontWeight: '500' } }, ' Suppr.')
          ),
          window.React.createElement('div', { style: { flex: 1, overflow: 'auto', padding: '12px' } },
            sel ? window.React.createElement('div', null,
              window.React.createElement('div', { style: { marginBottom: '12px', padding: '12px', backgroundColor: '#f9f9f9', border: '1px solid #e0e0e0', borderRadius: '4px' } },
                window.React.createElement('h5', { style: { margin: '0 0 8px 0', fontSize: '12px', fontWeight: 'bold' } }, sel.type.charAt(0).toUpperCase() + sel.type.slice(1)),
                window.React.createElement('div', { style: { fontSize: '10px', color: '#999', marginBottom: '8px' } }, 'ID: ' + sel.id.substring(0, 12) + '...'),
                window.React.createElement('div', { style: { display: 'grid', gap: '8px' } },
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '2px', color: '#555' } }, 'Position X'),
                    window.React.createElement('input', { type: 'number', value: Math.round(sel.x), onChange: (e) => { sel.x = parseFloat(e.target.value) || 0; triggerRender('property-edit'); }, style: { width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
                  ),
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '2px', color: '#555' } }, 'Position Y'),
                    window.React.createElement('input', { type: 'number', value: Math.round(sel.y), onChange: (e) => { sel.y = parseFloat(e.target.value) || 0; triggerRender('property-edit'); }, style: { width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
                  ),
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '2px', color: '#555' } }, 'Largeur'),
                    window.React.createElement('input', { type: 'number', value: Math.round(sel.w), onChange: (e) => { sel.w = Math.max(30, parseFloat(e.target.value) || 30); triggerRender('property-edit'); }, style: { width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
                  ),
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '2px', color: '#555' } }, 'Hauteur'),
                    window.React.createElement('input', { type: 'number', value: Math.round(sel.h), onChange: (e) => { sel.h = Math.max(30, parseFloat(e.target.value) || 30); triggerRender('property-edit'); }, style: { width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
                  ),
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '2px', color: '#555' } }, 'Rotation (°)'),
                    window.React.createElement('input', { type: 'number', min: '0', max: '360', value: Math.round(sel.rotation || 0), onChange: (e) => { sel.rotation = parseFloat(e.target.value) || 0; triggerRender('property-edit'); }, style: { width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px', boxSizing: 'border-box' } })
                  ),
                  window.React.createElement('div', null,
                    window.React.createElement('label', { style: { display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '4px', color: '#555' } }, 'Opacité'),
                    window.React.createElement('div', { style: { display: 'flex', gap: '4px', alignItems: 'center' } },
                      window.React.createElement('input', { type: 'range', min: '0', max: '1', step: '0.1', value: sel.opacity || 1, onChange: (e) => { sel.opacity = parseFloat(e.target.value); triggerRender('property-edit'); }, style: { flex: 1, cursor: 'pointer', height: '4px' } }),
                      window.React.createElement('span', { style: { fontSize: '11px', color: '#666', minWidth: '30px', textAlign: 'right' } }, Math.round((sel.opacity || 1) * 100) + '%')
                    )
                  )
                )
              )
            ) : window.React.createElement('div', { style: { textAlign: 'center', color: '#999', paddingTop: '20px' } },
              window.React.createElement('div', { style: { fontSize: '32px', marginBottom: '8px', opacity: '0.5' } }, ''),
              window.React.createElement('div', { style: { fontSize: '12px' } }, 'Sélectionnez un'),
              window.React.createElement('div', { style: { fontSize: '12px' } }, 'élément du canvas')
            )
          )
        );
      };
