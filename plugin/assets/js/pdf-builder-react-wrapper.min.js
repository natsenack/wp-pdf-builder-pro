/**
 * PDF Builder React Wrapper - V2 with V1 Smart Re-render System
 */
(function() {
  ...
  window.pdfBuilderReact = {
    initPDFBuilderReact: function(config) {
      ...
      const triggerRender = (reason = 'update') => {
        renderKey++;
        renderCount++;
        lastRenderTime = Date.now();
        console.log(`[PDF Builder] Render #${renderCount} - Reason: ${reason} - Elements: ${canvasElements.length}`);
        renderApp();
      };
      ...
      const Toolbar = function() {
        return window.React.createElement('div', { style: { display: 'flex', gap: '16px', padding: '12px 16px', backgroundColor: '#fff', border: '1px solid #e1e5e9', overflowX: 'auto' } },
          ...
          window.React.createElement('section', { style: { display: 'flex', flexDirection: 'column', gap: '6px', marginLeft: 'auto' } },
            ...
            window.React.createElement('div', { style: { fontSize: '11px', color: '#555' } }, canvasConfig.format + ' | ' + canvasElements.length + ' elem | #' + renderCount)
          )
        );
      };
        ...
        return window.React.createElement('div', { style: { flex: 1, backgroundColor: '#d8d8d8', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflow: 'auto' } },
          window.React.createElement('div', { 
            ...
            canvasElements.map((elem, idx) => {
              ...
              return window.React.createElement('div', { key: elem.id + '-' + renderCount,
                ...
                onClick: (e) => { e.stopPropagation(); selectedElementId = elem.id; triggerRender('element-select'); },
      ...
      const PropertiesPanel = function() {
        ...
        return window.React.createElement('div', { style: { width: '250px', backgroundColor: '#fff', borderLeft: '1px solid #ddd', overflow: 'auto', display: 'flex', flexDirection: 'column' } },
          window.React.createElement('div', { style: { padding: '10px', borderBottom: '1px solid #ddd', fontWeight: '600', fontSize: '12px', backgroundColor: '#f9f9f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
            window.React.createElement('span', null, ' Propriétés (#' + renderCount + ')'),
            sel && window.React.createElement('button', { onClick: () => { canvasElements = canvasElements.filter(e => e.id !== selectedElementId); selectedElementId = null; triggerRender('element-delete'); }, style: { padding: '4px 8px', border: '1px solid #dc3545', borderRadius: '3px', backgroundColor: '#dc3545', color: '#fff', cursor: 'pointer', fontSize: '10px', fontWeight: '500' } }, '')
          ),
              ...
              window.React.createElement('div', { style: { display: 'grid', gap: '8px' } },
                [{ label: 'X', key: 'x' }, { label: 'Y', key: 'y' }, { label: 'W', key: 'w' }, { label: 'H', key: 'h' }].map(p =>
                  window.React.createElement('div', { key: p.key, style: { display: 'grid', gridTemplateColumns: '30px 1fr', gap: '4px', alignItems: 'center' } },
                    ...
                    window.React.createElement('input', { type: 'number', value: Math.round(sel[p.key]), onChange: (e) => { sel[p.key] = parseFloat(e.target.value) || 0; triggerRender('property-edit'); }, style: { padding: '4px', border: '1px solid #ccc', borderRadius: '3px', fontSize: '11px' } })
                  )
                )
              )
