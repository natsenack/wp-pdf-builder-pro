...
(function($) {
    ...
    function setupEventListeners() {
        ...
        $(document).on('click', '.modal-close-btn, .modal-overlay', function(e) {
            if ($(this).hasClass('modal-overlay') || $(this).hasClass('modal-close-btn')) {
                closePreviewModal();
            }
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    alert('❌ Erreur: ' + (response.data?.message || 'Modèle non trouvé'));
                }
            },
    ...
    function saveTemplate() {
        ...
        const json = codeMirrorEditor ? codeMirrorEditor.getValue() : $('#template-json').val();
    ...
    function rotatePreview(degrees) {
        const previewImg = $('#template-preview-img');
        if (previewImg.length) {
            const currentRotation = parseInt(previewImg.css('transform').match(/rotate\(([^)]+)deg/)?.[1] || 0);
            previewImg.css('transform', 'rotate(' + (currentRotation + degrees) + 'deg)');
        }
    }
    ...
    function downloadPreview(format) {
        alert('Téléchargement en ' + format.toUpperCase() + ' (fonction à implémenter)');
    }
})(jQuery);
