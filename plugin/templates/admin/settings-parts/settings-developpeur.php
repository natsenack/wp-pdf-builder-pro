...
$devModeActive = $settings['pdf_builder_developer_enabled'] === '1';
$devSectionDisplay = $devModeActive ? 'block' : 'none';
?>
...
<div class="dev-tab-container">
    <!-- Status Banner -->
    <div class="dev-status-banner <?php echo $devModeActive ? 'active' : 'inactive'; ?>">
        <div style="font-size: 1.5em; margin-bottom: 5px;">🚀 Mode Développeur</div>
        <p style="margin: 0;"><?php echo $devModeActive ? 'Activé' : 'Désactivé'; ?></p>
        <div class="dev-quick-actions">
            <button type="button" class="button button-primary" id="dev-quick-enable" style="display: <?php echo $devModeActive ? 'none' : 'inline-block'; ?>;">⚡ Activer</button>
    ...
    <form method="post" id="settings-developpeur-form">
        ...
        <div class="dev-section" id="dev-section">
            ...
            <div class="dev-section-content">
                    ...
                    <div class="dev-card">
                        ...
                        <div style="margin-top: 15px;">
                            ...
                            <span style="margin-left: 10px;">
                                <?php echo $devModeActive ? 'Activé' : 'Désactivé'; ?>
                            </span>
                        </div>
                    </div>
                ...
                <!-- Section Outils - Conditionnellement visible selon le mode développeur -->
                <div class="dev-section" id="logs-tools-section" style="display: <?php echo $devSectionDisplay; ?>;">
                    <div class="dev-section-header">
        ...
        <div class="dev-section collapsed" id="todo-section">
            ...
            <div class="dev-section-content" aria-hidden="true">
                <div style="margin-bottom: 20px; display: flex; gap: 10px; align-items: center;">
                    <input type="text" id="new-todo-input" placeholder="Nouvelle tâche..." style="flex: 1; padding: 8px 12px; border: 1px solid #ddd; border-radius: 4px;" />
                    ...
                    <button type="button" id="add-todo-btn" class="button button-primary">➕ Ajouter</button>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const logsToolsSection = document.getElementById('logs-tools-section');

    function updateDeveloperMode() {
        if (!developerToggle || !logsToolsSection) return;

        const isEnabled = developerToggle.checked;
        console.log('[DEV MODE] Toggle changed:', isEnabled);

        // Mise à jour de l'affichage
        logsToolsSection.style.display = isEnabled ? 'block' : 'none';

        // Mise à jour du statut textuel
        const statusText = logsToolsSection.closest('.dev-tab-container').querySelector('.dev-status-banner p');
        if (statusText) {
            statusText.textContent = isEnabled ? 'Activé' : 'Désactivé';
        }

        const statusBanner = logsToolsSection.closest('.dev-tab-container').querySelector('.dev-status-banner');
        if (statusBanner) {
    ...
    function collectLogs() {
        ...
        const exampleLogs = [
            ...
            { type: 'error', message: '[PDF Builder] Erreur: Élément non trouvé', source: 'Canvas' }
        ];
    ...
    function displayLogs(logs) {
        ...
        html += `<div style="margin-top:15px; padding:10px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px;">
            <strong>Résumé:</strong> ${logs.length} logs
        </div>`;
    ...
    if (exportLogsBtn) {
        exportLogsBtn.addEventListener('click', function() {
            const logs = collectLogs();
            const data = { timestamp: new Date().toISOString(), logs: logs };
