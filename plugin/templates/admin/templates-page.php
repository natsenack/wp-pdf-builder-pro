...
<div class="wrap">
    ...
    <!-- Section de débogage -->
    <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 20px; margin: 20px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <h2 style="color: #856404; margin-top: 0;">🔧 Débogage API Templates</h2>

        <?php
        // Vérifier si la fonction existe
        echo '<h3>Fonction AJAX</h3>';
        ...
        echo '<h3>Table des templates</h3>';
        global $wpdb;
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_templates'") == $table_templates) {
            ...
            if ($count > 0) {
                ...
                $templates_debug = $wpdb->get_results("SELECT id, name, LENGTH(template_data) as data_length FROM $table_templates ORDER BY id", ARRAY_A);

                echo '<h4>Templates existants:</h4>';
                ...
                foreach ($templates_debug as $template) {
                    echo '<tr>';
                    echo '<td>' . $template['id'] . '</td>';
                    echo '<td>' . esc_html($template['name']) . '</td>';
                    echo '<td>' . $template['data_length'] . '</td>';
                    echo '<td>';
                    echo '<button onclick="testTemplateAPI(' . $template['id'] . ')" class="button button-small">Tester API</button> ';
                    echo '<a href="' . admin_url('admin.php?page=pdf-builder-react-editor&template_id=' . $template['id']) . '" class="button button-small button-primary">Éditer</a>';
                    echo '</td>';
                    echo '</tr>';
                }
                echo '</table>';
        ...
        <div id="api-test-result" style="margin-top: 15px; padding: 10px; background: white; border-radius: 4px; display: none;"></div>

        <script>
        function testTemplateAPI(templateId) {
            const resultDiv = document.getElementById('api-test-result');
            resultDiv.style.display = 'block';
            resultDiv.innerHTML = '🔄 Test de l\'API pour template ID ' + templateId + '...';

            // Créer un nonce (nous utilisons une valeur temporaire pour le test)
            const nonce = '<?php echo wp_create_nonce('pdf_builder_nonce'); ?>';

            fetch('<?php echo admin_url('admin-ajax.php'); ?>?action=pdf_builder_get_template&template_id=' + templateId + '&nonce=' + nonce)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        resultDiv.innerHTML = '<strong style="color: green;">✅ API fonctionne !</strong><br>' +
                            '<pre style="background: #f8f9fa; padding: 10px; border-radius: 4px; margin-top: 10px;">' +
                            JSON.stringify(data, null, 2) + '</pre>';
                    } else {
                        resultDiv.innerHTML = '<strong style="color: red;">❌ Erreur API:</strong> ' + (data.data || 'Erreur inconnue');
                    }
                })
                .catch(error => {
                    resultDiv.innerHTML = '<strong style="color: red;">❌ Erreur réseau:</strong> ' + error.message;
                });
        }
        </script>
    </div>
