...
// Pour cohérence de la clé
    ...
    public static function invalidateOrderCache(int $orderId): void
    {
        ...
        if (isset($GLOBALS['wpdb'])) {
            global $wpdb;
            $transient_keys = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '_transient_' . self::CACHE_PREFIX . 'order_' . $orderId . '*'));
    ...
    public static function invalidateCompanyCache(string $context = ''): void
    {
        ...
        // Invalider tous les caches entreprise
            if (isset($GLOBALS['wpdb'])) {
                global $wpdb;
                $transient_keys = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '_transient_' . self::CACHE_PREFIX . 'company_%'));
                foreach ($transient_keys as $transient_key) {
                    $key = str_replace('_transient_', '', $transient_key);
                    delete_transient($key);
                }
...
// Invalider les caches de commandes qui pourraient contenir ce produit
        ...
        if (isset($GLOBALS['wpdb'])) {
            global $wpdb;
            $order_transient_keys = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s LIMIT 50", '_transient_' . self::CACHE_PREFIX . 'order_%'));
    ...
    public static function clearAllCache(): void
    {
        if (isset($GLOBALS['wpdb'])) {
            global $wpdb;
            $transient_keys = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '_transient_' . self::CACHE_PREFIX . '%'));
            foreach ($transient_keys as $transient_key) {
                $key = str_replace('_transient_', '', $transient_key);
                delete_transient($key);
            }
...
// Supprimer les transients expirés
        $cleaned = $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->options} WHERE option_name LIKE %s AND option_value < %d", '_transient_timeout_' . self::CACHE_PREFIX . '%', time()));
        return $cleaned ?: 0;
    }
