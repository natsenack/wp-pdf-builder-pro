...
// Hook pour les avertissements de performance
     ...
     */
    private function getNotificationLevel()
    {
     ...
     */
    private function shouldSendNotification($level)
    {
        $current_level = $this->getNotificationLevel();
        $levels = array(
            'error' => 1,
            'warning' => 2,
            'info' => 3
     ...
     */
    private function sendNotification($subject, $message, $level = 'info')
    {
...
// Vérifier si SMTP est activé
        ...
        if ($smtp_enabled) {
            ...
            $smtp_result = $this->sendSmtpEmail($to, $subject, $message);
...
// Logger l'envoi
     ...
     */
    public function notifyPdfGenerated($order_id, $template_id)
    {
        ...
        $message = sprintf(__("Bonjour,\n\nUn PDF a été généré avec succès.\n\nDétails :\n- Commande : #%s\n- Template : %s\n- Date : %s\n\nCordialement,\nL'équipe PDF Builder Pro", 'pdf-builder-pro'), $order_id, get_the_title($template_id), current_time('d/m/Y H:i:s'));
     ...
     */
    public function notifyPdfError($order_id, $error_message)
    {
     ...
     */
    public function notifyTemplateDeleted($template_id, $template_name)
    {
        ...
        $message = sprintf(__("Bonjour,\n\nUn template a été supprimé.\n\nDétails :\n- Template : %s (ID: %s)\n- Supprimé par : %s\n- Date : %s\n\nCordialement,\nL'équipe PDF Builder Pro", 'pdf-builder-pro'), $template_name, $template_id, wp_get_current_user()->display_name, current_time('d/m/Y H:i:s'));
...
// Vérifier si SMTP est activé
        ...
        return $this->sendNotification($subject, $message, 'info');
    }
