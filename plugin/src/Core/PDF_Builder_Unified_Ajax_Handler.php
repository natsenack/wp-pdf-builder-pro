...
class PDF_Builder_Unified_Ajax_Handler {
     ...
     */
      ...
      */
     public function handle_save_canvas_settings() {
         if (!$this->nonce_manager->validate_ajax_request('canvas_settings')) {
             return;
         }
         ...
         try {
             ...
             $canvas_params = [
                 ...
                 'pdf_builder_canvas_error_reporting',
                 'pdf_builder_canvas_memory_limit_php',
                 'pdf_builder_canvas_allow_portrait',
                 'pdf_builder_canvas_allow_landscape'
             ];

             foreach ($canvas_params as $param) {
                 if (isset($_POST[$param])) {
                     $value = $_POST[$param];

                     // Gérer les tableaux (comme pdf_builder_canvas_dpi[])
                     if (is_array($value)) {
                         $value = implode(',', $value);
                     }

                     // Sauvegarder dans la table personnalisée
                     if (pdf_builder_update_option($param, sanitize_text_field($value))) {
                         $saved_count++;
                         $saved_options[] = $param;
                     }
                 }
             ...
             if (class_exists('PDF_Builder_Logger')) {
                 PDF_Builder_Logger::get_instance()->debug_log('[PDF Builder AJAX] Canvas settings saved: ' . $saved_count . ' options - ' . implode(', ', $saved_options));
             }

             wp_send_json_success([
                 'message' => "$saved_count paramètre(s) canvas sauvegardé(s) avec succès.",
                 'saved_count' => $saved_count,
             ...
             if (class_exists('PDF_Builder_Logger')) {
                 PDF_Builder_Logger::get_instance()->debug_log('[PDF Builder AJAX] Erreur sauvegarde canvas: ' . $e->getMessage());
             }
             wp_send_json_error(['message' => 'Erreur lors de la sauvegarde des paramètres canvas']);
         }
     }
