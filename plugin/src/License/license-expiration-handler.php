...
class License_Expiration_Handler {
        ...
        if (!empty($license_expires) && $license_status !== 'free') {
            ...
            if ($now > $expires_date) {
                ...
                if (in_array($diff_days, [30, 7])) {
                    self::send_expiration_notification($license_expires, $diff_days);
                }
            }
        }
     ...
     */
    private static function send_expiration_notification($expiration_date, $days_remaining) {
        ...
        $subject = sprintf('[%s] Votre licence PDF Builder Pro expire dans %d jour%s', 
            $site_name, 
            $days_remaining, 
            $days_remaining > 1 ? 's' : ''
        );
        ...
        $message = sprintf(
            '<h2>Notification d\'expiration de licence</h2>' .
            '<p>Bonjour,</p>' .
            ...
            'Vous pouvez désactiver les notifications dans l\'onglet Licence des paramètres de PDF Builder Pro.</em></p>',
            $site_name,
            $days_remaining,
            $days_remaining > 1 ? 's' : '',
            date('d/m/Y', strtotime($expiration_date)),
            $site_url,
            date('d/m/Y', strtotime($expiration_date)),
            $days_remaining,
            admin_url('admin.php?page=pdf-builder-pro-settings&tab=licence')
        );
