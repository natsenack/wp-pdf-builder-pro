 ...
 * @since 1.0.0
 */
class PDF_Builder_JSON_Optimizer {
     ...
     * @since 1.0.0
     */
    public static function minify_json($data) {
    ...
    public static function compress($json_data) {
        ...
        if (strlen($json_data) < self::$compression_threshold) {
            return $json_data; // Pas assez gros pour compresser
        }
        
        if (!function_exists('gzcompress')) {
            return $json_data; // GZIP non disponible
        }
        
        $compressed = gzcompress($json_data, 9);
        if ($compressed === false) {
            return $json_data; // Erreur compression
        }
     ...
     * @since 1.0.0
     */
    public static function deduplicate_values($data) {
        if (!is_array($data)) {
            return $data;
        }
        ...
        $result = self::_deduplicate_recursive($data, $value_map, $next_ref_id);
     ...
     * @private
     */
    private static function _deduplicate_recursive(&$data, &$value_map, &$ref_id) {
        if (!is_array($data)) {
            return $data;
        }
        ...
        foreach ($data as $key => &$value) {
            if (is_array($value)) {
                $value = self::_deduplicate_recursive($value, $value_map, $ref_id);
     ...
     * @since 1.0.0
     */
    public static function get_compression_stats($original_data, $compressed_data = null) {
     ...
     * @since 1.0.0
     */
    public static function optimize_template($template_data) {
        // Minifier le JSON
        $json_minified = self::minify_json($template_data);
        
        // Compresser si bénéfique
        $json_compressed = self::compress($json_minified);
        
        // Calculer stats
        $stats = self::get_compression_stats($template_data, $json_compressed);
     ...
     * @since 1.0.0
     */
    public static function set_compression_threshold($threshold) {
