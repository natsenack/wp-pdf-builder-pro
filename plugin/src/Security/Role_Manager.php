...
class Role_Manager {
    ...
    public static function init() {
        add_action('plugins_loaded', [__CLASS__, 'register_capabilities'], 10);
        add_filter('user_has_cap', [__CLASS__, 'check_pdf_builder_capability'], 10, 4);
    }
     ...
     */
    public static function check_pdf_builder_capability($allcaps, $cap, $args, $user) {
     ...
     */
    public static function user_can_access_pdf_builder() {
     ...
     */
    public static function user_has_access($user) {
        if (!$user) {
            return false;
        }
    ...
    public static function check_and_block_access() {
        if (!current_user_can('manage_options') && !self::user_can_access_pdf_builder()) {
            wp_die(
