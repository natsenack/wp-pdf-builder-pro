        ...
        foreach ($altPaths as $path) {
            if (file_exists($path)) {
                ...
                try {
                    ...
                    if (class_exists('Dompdf\Dompdf')) {
                        $this->logInfo(
                            "DomPDF loaded successfully from: $path - New classes: " .
                            implode(', ', array_slice($newClasses, -5))
                        );
                        return true;
                    }
                    ...
                    if (class_exists('Spipu\Html2Pdf\Html2Pdf')) {
                        $this->logInfo(
                            "HTML2PDF loaded successfully from: $path - New classes: " .
                            implode(', ', array_slice($newClasses, -5))
                        );
                        return true;
                    }
                    ...
                    $this->logWarning(
                        "Library loaded from $path but expected classes not found. " .
                        "Available classes: " . implode(', ', $newClasses)
                    );
                } catch (\Exception $e) {
...
// Générer d'abord le PDF/HTML
     ...
     */
    private function convertPdfFileToImage(string $pdf_path, int $quality, string $format): string
    {
     ...
     */
    private function convertPdfWithImagick(string $pdf_path, int $quality, string $format): string
    {
        try {
            $imagick = new \Imagick();
...
// Optimisations selon format
     ...
     */
    private function convertHtmlToImage(string $html, int $quality, string $format): string
    {
