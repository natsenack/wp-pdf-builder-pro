...
function pdf_builder_save_company_data($data) {
    try {
        error_log('PDF Builder Wizard: pdf_builder_save_company_data called with data: ' . print_r($data, true));
        ...
        error_log('PDF Builder Wizard: Exception in pdf_builder_save_company_data: ' . $e->getMessage());
        return array('success' => false, 'message' => 'Erreur lors de la sauvegarde: ' . $e->getMessage());
    }
}

/**
 * Créer le template par défaut (fonction standalone)
 */
function pdf_builder_create_default_template() {
    try {
        global $wpdb;
