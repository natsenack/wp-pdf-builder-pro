...
?>
            ...
            function initializeCanvasPreviews() {
                console.log('🎨 Initialisation des previews canvas...');
                ...
                // Fonction pour obtenir une valeur avec fallback par défaut
                function getValueWithDefault(selector, optionKey, defaultValue) {
                    const element = document.querySelector(selector);
                    if (element && element.value) {
                        return element.value;
                    }

                    // Essayer de récupérer depuis les options WordPress (si disponible)
                ...
                // Initialiser les previews de dimensions
                const widthEl = document.getElementById('card-canvas-width');
                const heightEl = document.getElementById('card-canvas-height');
                const dpiEl = document.getElementById('card-canvas-dpi');

                if (widthEl && !widthEl.textContent.trim()) {
                    widthEl.textContent = defaults.width;
                }
                if (heightEl && !heightEl.textContent.trim()) {
                    heightEl.textContent = defaults.height;
                }
                if (dpiEl && !dpiEl.textContent.trim()) {
                    const widthMM = ((defaults.width / defaults.dpi) * 25.4).toFixed(1);
                    const heightMM = ((defaults.height / defaults.dpi) * 25.4).toFixed(1);
                    dpiEl.textContent = `${defaults.dpi} DPI - ${defaults.format} (${widthMM}×${heightMM}mm)`;
                }

                // Initialiser les previews d'apparence
                const bgPreview = document.querySelector('.apparence-background');
                const borderPreview = document.querySelector('.apparence-border');
                const shadowPreview = document.querySelector('.apparence-shadow');

                if (bgPreview && !bgPreview.style.backgroundColor) {
                    bgPreview.style.backgroundColor = defaults.bgColor;
                }
                if (borderPreview && !borderPreview.style.border) {
                    borderPreview.style.border = `${defaults.borderWidth}px solid ${defaults.borderColor}`;
                }
                if (shadowPreview && defaults.shadowEnabled === '1') {
                    shadowPreview.style.display = 'block';
                }

                // Initialiser les previews de grille
                const gridPreview = document.querySelector('.grid-preview-pattern');
                if (gridPreview && defaults.gridEnabled === '1') {
                    gridPreview.style.backgroundImage = `
                        linear-gradient(to right, ${defaults.gridColor} 1px, transparent 1px),
                        linear-gradient(to bottom, ${defaults.gridColor} 1px, transparent 1px)
                    `;
                    gridPreview.style.backgroundSize = `${defaults.gridSize}px ${defaults.gridSize}px`;
                }
