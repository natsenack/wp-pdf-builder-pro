<?php
/**
 * Test complet de persistance des propriétés d'éléments
 * Vérifie que toutes les propriétés sont sauvegardées et chargées correctement
 */
...
echo "<h1>🧪 Test complet de persistance des propriétés</h1>";
...
// Test 1: Test de sérialisation/désérialisation
...
// Test 2: Test de nettoyage pour sauvegarde (simuler la fonction JavaScript)
...
// Test 3: Test de sauvegarde/chargement depuis la base de données
        ...
        if ($loadedTemplate) {
            ...
            if ($loadedData && isset($loadedData['elements'][0])) {
                ...
                foreach ($examples as $example) {
                    if (isset($loadedElement[$example])) {
                        $original = $cleanedElement[$example] ?? 'N/A';
                        $loaded = $loadedElement[$example];
                        $status = $original === $loaded ? '✅' : '❌';
                        echo "<li><strong>$example:</strong> '$loaded' $status</li>";
                    }
                }
                echo "</ul>";
