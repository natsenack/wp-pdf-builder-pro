...
class PDF_Builder_Canvas_Elements_Manager {
    ...
    public function validate_and_clean_json_elements($json_string, $source = 'unknown') {
        ...
        pdf_builder_debug("Validation JSON depuis {$source} - Longueur: " . strlen($json_string), 2, 'json_validation');

        // Vérifier si la chaîne JSON est vide ou nulle
        if (empty($json_string) || $json_string === 'null') {
            pdf_builder_debug("Chaîne JSON vide ou nulle depuis {$source}", 1, 'json_validation');
            return [];
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            pdf_builder_debug("Erreur de décodage JSON depuis {$source}: {$error_msg}", 1, 'json_validation');
            ...
            if ($repaired_json !== false) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    pdf_builder_debug("JSON réparé avec succès depuis {$source}", 2, 'json_validation');
                } else {
                    pdf_builder_debug("Échec de réparation JSON depuis {$source}", 1, 'json_validation');
        ...
        if (!is_array($elements)) {
            pdf_builder_debug("Données JSON non-tableau depuis {$source}", 1, 'json_validation');
        ...
        foreach ($elements as $index => $element) {
            ...
            if (!empty($element_errors)) {
                $errors = array_merge($errors, array_map(function($err) use ($index) {
        ...
        if (!empty($errors)) {
            pdf_builder_debug("Erreurs de validation JSON depuis {$source}: " . implode('; ', $errors), 1, 'json_validation');
        }

        pdf_builder_debug(count($cleaned_elements) . " éléments validés et nettoyés depuis {$source}", 2, 'json_validation');
    ...
    private function get_template_elements_from_db($template_id) {
        ...
        if (is_wp_error($elements)) {
            pdf_builder_debug('Erreur lors du chargement des éléments du template ' . $template_id . ': ' . $elements->get_error_message(), 1, 'elements_manager');
            return [];
        }
    ...
    public function load_canvas_elements($template_id) {
        ...
        if (isset($template_data['elements']) && is_array($template_data['elements'])) {
            ...
            if (is_wp_error($validated_elements)) {
                error_log('PDF_Builder_Canvas_Elements_Manager: Erreur validation éléments template ' . $template_id . ': ' . $validated_elements->get_error_message());
