...
### ⚡ DÉPLOIEMENT ULTRA-RAPIDE (8 connexions simultanées)

**Commande principale :**
```powershell
# Compiler + Déployer en une commande
npm run build && .\deploy-simple.ps1
```

**Commandes séparées :**
```powershell
# 1. Compiler les assets TypeScript/React
npm run build

# 2. Déployer via FTP ultra-rapide (17-20 secondes)
.\deploy-simple.ps1
```

**Configuration FTP :**
- Modifiez `ftp-config.env` pour vos paramètres FTP
- Le script utilise 8 connexions simultanées pour la vitesse maximale
- Configuration des fichiers dans `deploy-files.config`

**Fichiers déployés automatiquement :**
- ✅ Tous les `*.php`
- ✅ `assets/css/**`, `assets/js/**`, `assets/images/**`
- ✅ `includes/**`, `languages/**`
- ❌ `node_modules/**`, fichiers de dev, logs, etc.

Le script `deploy.bat` :
- Compile automatiquement les assets
- Crée un dossier `deploy-production/` avec uniquement les fichiers nécessaires
- Exclut automatiquement node_modules/ et les fichiers de développement
- Affiche la taille du déploiement
...
# 1. Éditez ftp-config.env avec vos identifiants FTP
# 2. Lancez le déploiement automatique

./deploy-full.bat
```

Le script détecte automatiquement la présence de `ftp-config.env` et :
- ✅ Compile et prépare le déploiement local
- ✅ Upload automatiquement via FTP si configuré
- ✅ Affiche la progression détaillée

#### Configuration FTP (fichier `ftp-config.env`) :
```env
FTP_SERVER=votre-serveur-ftp.com
FTP_USERNAME=votre-username
FTP_PASSWORD=votre-mot-de-passe
FTP_REMOTE_PATH=/wp-content/plugins/wp-pdf-builder-pro
DEPLOY_LOCAL_DIR=deploy-production
```

💡 **Astuce** : Copiez `ftp-config.env.example` vers `ftp-config.env` et éditez-le.
...
├── 📁 assets/          ← CSS/JS compilés uniquement
├── 📁 includes/        ← Code PHP
├── 📁 languages/       ← Traductions
├── 📁 dist/           ← Assets frontend compilés
├── 📄 pdf-builder-pro.php
└── 📄 README.md
```
