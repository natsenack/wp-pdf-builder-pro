...
class PDF_Builder_Admin {
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        // Scripts JavaScript
        wp_enqueue_script('pdf-builder-admin', PDF_BUILDER_PRO_ASSETS_URL . 'js/dist/pdf-builder-admin.js', ['jquery', 'wp-api'], PDF_BUILDER_PRO_VERSION, true);
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        // Forcer le rechargement du cache avec timestamp
        $version = time();

        // Styles CSS de base
        wp_enqueue_style('pdf-builder-admin', PDF_BUILDER_PRO_ASSETS_URL . 'css/pdf-builder-admin.css', [], $version);
        wp_enqueue_style('pdf-builder-canvas', PDF_BUILDER_PRO_ASSETS_URL . 'css/pdf-builder-canvas.css', [], $version);

        // Scripts JavaScript
        wp_enqueue_script('pdf-builder-admin', PDF_BUILDER_PRO_ASSETS_URL . 'js/dist/pdf-builder-admin.js', ['jquery', 'wp-api'], $version, true);
        wp_enqueue_script('pdf-builder-canvas', PDF_BUILDER_PRO_ASSETS_URL . 'js/pdf-builder-canvas.js', ['jquery', 'wp-api'], $version, true);

        // Scripts utilitaires
        wp_enqueue_script('pdf-builder-utils', PDF_BUILDER_PRO_ASSETS_URL . 'js/dist/pdf-builder-utils.js', ['jquery'], $version, true);
        wp_enqueue_script('pdf-builder-unified-config', PDF_BUILDER_PRO_ASSETS_URL . 'js/dist/pdf-builder-unified-config.js', ['jquery'], $version, true);
        ...
        // Styles pour l'éditeur canvas
        if ($hook === 'pdf-builder_page_pdf-builder-editor') {
            wp_enqueue_style('pdf-builder-canvas-editor', PDF_BUILDER_PRO_ASSETS_URL . 'css/pdf-builder-canvas.css', [], $version);
        }
    }
}
