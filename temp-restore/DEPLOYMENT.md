...
Prépare les fichiers pour le déploiement en créant un dossier `deploy/` avec tous les fichiers nécessaires :
- `dist/` - Fichiers JavaScript/TypeScript compilés
- `assets/` - Ressources CSS/JS/images
- `languages/` - Fichiers de traduction
- Fichiers PHP et Markdown du plugin

### `.\ftp-deploy-optimized.ps1`
Déploie les fichiers via FTP avec optimisation :
1. **Crée automatiquement une archive locale** dans le dossier `archive/` avec timestamp
2. **Déploie les fichiers** via FTP avec gestion d'erreurs
3. **Archive tous les fichiers** qui seront déployés

## Configuration FTP

Avant d'utiliser le script de déploiement, configurez vos paramètres FTP :

### Option 1 : Variables d'environnement
```powershell
$env:FTP_HOST = "ftp.votresite.com"
$env:FTP_USER = "votre_username"
$env:FTP_PASSWORD = "votre_mot_de_passe"
$env:FTP_PATH = "/wp-content/plugins/wp-pdf-builder-pro"
```
