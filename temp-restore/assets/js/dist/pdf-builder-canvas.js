/**
 * PDF Builder Pro - Canvas Nettoyé et Optimisé
 * Version 6.0.0 - Système de canvas simplifié et performant
 */
...
const PDF_BUILDER_CANVAS = {
    ...
    init: function(options = {}) {
        ...
        // Appliquer les dimensions dynamiques
        ...
        this.layers.create('default', { name: 'Calque par défaut' });
        this.drawing.init();
        this.performance.startMonitoring();

        // Activer l'IA si configurée
        if (this.state.ai && this.state.ai.enabled) {
            this.ai.analyzeLayout();
        }

        this.state.initialized = true;
        console.log('🚀 Canvas complexe initialisé avec IA et collaboration');
    },
    ...
    setupCanvas: function() {
        ...
        console.log('🔧 Configuration du canvas de base...');
        ...
        console.log('🔧 Canvas HTML structure created');
        console.log('🔧 Canvas content:', $canvas.html().substring(0, 100) + '...');

        this.updateCanvasSize();

        console.log('🔧 Canvas dimensions after:', $canvas.width(), 'x', $canvas.height());
        console.log('🔧 Canvas CSS width:', $canvas.css('width'));
        console.log('🔧 Canvas CSS height:', $canvas.css('height'));
        console.log('🔧 Canvas visible:', $canvas.is(':visible'));
    },
    ...
    setupGrid: function() {
        ...
        // Génération optimisée de la grille
        let gridHTML = '';
    ...
    bindEvents: function() {
        ...
        // Drag & drop d'éléments depuis la palette
        ...
        jQuery(document).on('dragleave', '#pdf-canvas', function(e) {
            ...
            // Vérifier si on quitte vraiment le canvas (pas un enfant)
            if (e.target === this || !jQuery(e.target).closest('#pdf-canvas').length) {
