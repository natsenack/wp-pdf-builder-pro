...
if ($All) {
    ...
    # Essayer d'utiliser git si disponible
    $gitAvailable = $null
    try {
        $gitAvailable = @(& git status 2>&1 | Select-Object -First 1)
        if ($gitAvailable -match "fatal" -or $gitAvailable -match "not a git repository") {
            $gitAvailable = $null
        }
    } catch {
        $gitAvailable = $null
    ...
    if ($gitAvailable) {
        Write-Log "Utilisation de Git pour détecter les fichiers modifiés" "INFO"
        try {
            ...
            Write-Log "Erreur lors de la lecture de Git, utilisation du fallback timestamp" "WARN"
            $gitAvailable = $null
        }
    }
    
    # Fallback: si pas de git ou erreur, utiliser les fichiers critiques + modifiés récemment
    if (-not $gitAvailable -or $filesToDeploy.Count -eq 0) {
        Write-Log "Mode Fallback: fichiers critiques + modifiés dans les 24 dernières heures" "INFO"
