...
class PdfBuilderCore
{
    ...
    private function loadDependencies()
    {
        ...
        foreach ($managers as $manager) {
            $manager_path = PDF_BUILDER_PLUGIN_DIR . 'src/backend/utilities/Managers/' . $manager;
        ...
        foreach ($core_classes as $core_class) {
            $core_path = PDF_BUILDER_PLUGIN_DIR . 'src/backend/core/Core/' . $core_class;
        ...
        if (file_exists(PDF_BUILDER_PLUGIN_DIR . 'src/backend/admin/Admin/PDF_Builder_Admin.php')) {
            require_once PDF_BUILDER_PLUGIN_DIR . 'src/backend/admin/Admin/PDF_Builder_Admin.php';
        }
        ...
        if (!did_action('plugins_loaded')) {
            ...
            add_action('plugins_loaded', function() {
                if (function_exists('pdf_builder_is_woocommerce_active') && pdf_builder_is_woocommerce_active() && file_exists(PDF_BUILDER_PLUGIN_DIR . 'src/backend/utilities/Controllers/PDF_Generator_Controller.php')) {
                    require_once PDF_BUILDER_PLUGIN_DIR . 'src/backend/utilities/Controllers/PDF_Generator_Controller.php';
                }
            }, 5);
        }

        if ($load_pdf_controller_now && file_exists(PDF_BUILDER_PLUGIN_DIR . 'src/backend/utilities/Controllers/PDF_Generator_Controller.php')) {
            require_once PDF_BUILDER_PLUGIN_DIR . 'src/backend/utilities/Controllers/PDF_Generator_Controller.php';
        }

        // Charger le handler AJAX d'image de prévisualisation
        if (file_exists(PDF_BUILDER_PLUGIN_DIR . 'src/backend/api/AJAX/preview-image-handler.php')) {
            require_once PDF_BUILDER_PLUGIN_DIR . 'src/backend/api/AJAX/preview-image-handler.php';
        }
    }
    ...
    private function enqueueReactGlobalAPI()
    {
        ...
        $react_script_url = PDF_BUILDER_PLUGIN_URL . 'resources/assets/js/pdf-builder-react.js';
        $cache_bust = time();
    ...
    private function initAdmin()
    {
        ...
        if (!class_exists('PDF_Builder\Admin\PdfBuilderAdmin')) {
            include_once PDF_BUILDER_PLUGIN_DIR . 'src/backend/admin/Admin/PDF_Builder_Admin.php';
        }
        ...
        <script type="text/javascript">
            ...
            try {
                window.pdfBuilderData = {
                    ...
                    nonce: '<?php echo wp_create_nonce('pdf_builder_save_template_nonce'); ?>'
                };
            } catch (e) {
