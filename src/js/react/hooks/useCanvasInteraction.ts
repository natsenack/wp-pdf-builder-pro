...
export const useCanvasInteraction = ({
  ...
  const pendingRotationUpdateRef = useRef<{ x: number; y: number } | null>(
    null,
  );
  ...
  const requestAnimationFrameLimited = useCallback(
    (callback: () => void) => {
        ...
        if (
          ...
          selectedElementIds = currentState.elements
            .filter((element) => {
              ...
              const rectRight =
                selectionRectRef.current.x + selectionRectRef.current.width;
              const rectBottom =
                selectionRectRef.current.y + selectionRectRef.current.height;
              return !(
                element.x > rectRight ||
                elementRight < selectionRectRef.current.x ||
                element.y > rectBottom ||
                elementBottom < selectionRectRef.current.y
              );
            })
            .map((element) => element.id);
        }
  ...
  const performRotationUpdate = useCallback(() => {
    ...
    const { x: currentMouseX, y: currentMouseY } =
    ...
    const selectedElements = lastState.elements.filter((el) =>
    ...
    const startAngle = Math.atan2(
      rotationMouseStartRef.current.y - centerY,
      rotationMouseStartRef.current.x - centerX,
    );
  ...
  const getResizeHandleAtPosition = useCallback(
    (x: number, y: number, selectedIds: string[], elements: Element[]) => {
      const handleSize = 8;
      ...
      for (const element of selectedElements) {
        ...
        for (const handle of handles) {
          ...
          if (
            x >= handle.x - handleMargin &&
            x <= handle.x + handleSize + handleMargin &&
            y >= handle.y - handleMargin &&
            y <= handle.y + handleSize + handleMargin
          ) {
  ...
  useEffect(() => {
    const { fpsTarget, performanceMonitoring, debugMode, memoryLimitJs } =
      canvasSettings;
    ...
    if (performanceMonitoring || debugMode) {
      const updateFPS = () => {
        ...
        if (now - lastFrameTimeRef.current >= 1000) {
          performanceMetricsRef.current.fps = Math.round(
          ...
          if (performanceMonitoring && "memory" in performance) {
            ...
            performanceMetricsRef.current.memoryUsage = Math.round(
            ...
            if (memory.usedJSHeapSize > memoryLimitJs * 1024 * 1024) {
              debugWarn(
                `[Canvas Performance] Mémoire JS dépassée: ${performanceMetricsRef.current.memoryUsage}MB / ${memoryLimitJs}MB`,
              );
            }
          }
      ...
      if (debugMode) {
        debugLog(
          `[Canvas Performance] Monitoring activé - FPS cible: ${fpsTarget}, Mémoire limite: ${memoryLimitJs}MB`,
        );
      }
    }
  ...
  const handleMouseDown = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (!validateCanvasRect(rect)) {
        debugLog(
          "[CanvasInteraction] Mouse down ignored - invalid canvas rect",
        );
        return;
      }
      ...
      debugLog(
        `[CanvasInteraction] Mouse down at canvas coords (${x.toFixed(1)}, ${y.toFixed(1)}), zoom: ${zoomScale}, pan: (${state.canvas.pan.x.toFixed(1)}, ${state.canvas.pan.y.toFixed(1)})`,
      );
      ...
      if (clickedElement) {
        debugLog(
          `[CanvasInteraction] Clicked element: ${clickedElement.type} (${clickedElement.id})`,
        );
        ...
        const isMultiSelect =
          canvasSettings.selectionMultiSelectEnabled && event.ctrlKey;

        if (isMultiSelect) {
          debugLog(
            `[CanvasInteraction] Multi-select mode - ${isAlreadySelected ? "removing" : "adding"} element ${clickedElement.id}`,
          );
          // ✅ Mode sélection multiple
          if (isAlreadySelected) {
            // Retirer l'élément de la sélection
            const newSelection = state.selection.selectedElements.filter(
          ...
          if (!isAlreadySelected) {
            ...
            dragStartRef.current = {
              [clickedElement.id]: { x: clickedElement.x, y: clickedElement.y },
            };
      ...
      const resizeHandle = getResizeHandleAtPosition(
        x,
        y,
        state.selection.selectedElements,
        state.elements,
      ...
      if (resizeHandle) {
        debugLog(
          `[CanvasInteraction] Starting resize - element: ${resizeHandle.elementId}, handle: ${resizeHandle.handle}`,
        );
      ...
      if (
        canvasSettings?.selectionRotationEnabled !== false &&
        state.selection.selectedElements.length > 0
      ) {
        ...
        if (selectedElements.length > 0) {
          ...
          const distance = Math.sqrt(
            (x - centerX) ** 2 + (y - rotationHandleY) ** 2,
          );
          if (distance <= rotationHandleSize / 2) {
            debugLog(
              `[CanvasInteraction] Starting rotation for ${state.selection.selectedElements.length} elements`,
            );
      ...
      if (selectionMode === "lasso" || selectionMode === "rectangle") {
        debugLog(
          `[CanvasInteraction] Starting ${selectionMode} selection at (${x.toFixed(1)}, ${y.toFixed(1)})`,
        );
        ...
        if (state.selection.selectedElements.length > 0) {
          debugLog(
            `[CanvasInteraction] Clearing selection (${state.selection.selectedElements.length} elements)`,
          );
          dispatch({ type: "CLEAR_SELECTION" });
  ...
  const handleMouseUp = useCallback(() => {
    debugLog(
      `[CanvasInteraction] Mouse up - ending interactions (dragging: ${isDraggingRef.current}, resizing: ${isResizingRef.current}, rotating: ${isRotatingRef.current}, selecting: ${isSelectingRef.current})`,
    );
    ...
    if (isSelectingRef.current) {
      ...
      if (selectionMode === "lasso" && selectionPointsRef.current.length > 2) {
        ...
        selectedElementIds = state.elements
          ...
          .map((element) => element.id);
        debugLog(
          `[CanvasInteraction] Lasso selection completed - ${selectedElementIds.length} elements selected`,
        );
      } else if (
        selectionMode === "rectangle" &&
        selectionRectRef.current.width > 0 &&
        ...
        selectedElementIds = state.elements
          ...
          .map((element) => element.id);
        debugLog(
          `[CanvasInteraction] Rectangle selection completed - ${selectedElementIds.length} elements selected`,
        );
      }
      ...
      if (selectedElementIds.length > 0) {
        debugLog(
          `[CanvasInteraction] Applying selection: ${selectedElementIds.join(", ")}`,
        );
        dispatch({ type: "SET_SELECTION", payload: selectedElementIds });
      } else {
        debugLog(
          `[CanvasInteraction] No elements selected - clearing selection`,
        );
        dispatch({ type: "CLEAR_SELECTION" });
      }
  ...
  const getCursorAtPosition = useCallback(
    (x: number, y: number): string => {
      ...
      if (state.selection.selectedElements.length > 0) {
        const elementUnderMouse = state.elements.find(
          (el) =>
            state.selection.selectedElements.includes(el.id) &&
  ...
  const calculateResize = useCallback(
    (
      ...
      // ✅ NOUVELLE: Récupérer les dimensions minimales basées sur le contenu
      const getMinDimensions = (el: Element) => {
      ...
      const minDimensions = getMinDimensions(element);
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (isDraggingRef.current && selectedElementsRef.current.length > 0) {
        ...
        debugLog(
          `[CanvasInteraction] Resizing element ${selectedElementRef.current} with handle ${resizeHandleRef.current} at (${x.toFixed(1)}, ${y.toFixed(1)})`,
        );

        // ✅ BALANCED: Preserve essential properties without overkill
        const lastState = lastKnownStateRef.current;
        const element = lastState.elements.find(
          (el) => el.id === selectedElementRef.current,
        );
        if (!element) return;

        const resizeUpdates = calculateResize(
          element,
          resizeHandleRef.current,
          x,
          y,
        ...
        debugLog(
          `[CanvasInteraction] Rotating ${state.selection.selectedElements.length} elements at mouse position (${x.toFixed(1)}, ${y.toFixed(1)})`,
        );
        ...
        if (rafIdRef.current === null) {
          rafIdRef.current = requestAnimationFrameLimited(
            performRotationUpdate,
          );
        }
      }
  ...
  const handleContextMenu = useCallback(
    (
      ...
      const clickedElement = state.elements.find((el) =>
        isPointInElement(canvasX, canvasY, el),
      );
  ...
  const isElementInLasso = useCallback(
    (element: Element, points: { x: number; y: number }[]): boolean => {
