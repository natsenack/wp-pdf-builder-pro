...
export const SaveTooltip: FC<SaveTooltipProps> = ({
  ...
  return (
    <div
      className={`pdfb-save-tooltip pdfb-save-tooltip--${state}`}
      onMouseEnter={() => setIsHovering(true)}
      onMouseLeave={() => setIsHovering(false)}
    >
      {/* Indicateur (petit point/icône) */}
      <div className="pdfb-save-tooltip__indicator">
        {state === 'saving' && (
          <div className="pdfb-save-tooltip__spinner">
            <div className="pdfb-save-tooltip__spinner-dot"></div>
          </div>
        )}
        {state === 'saved' && (
          <svg className="pdfb-save-tooltip__icon" width="16" height="16" viewBox="0 0 20 20" fill="none">
      ...
      {/* Tooltip */}
      {isHovering && (
        <div className="pdfb-save-tooltip__popup">
          <div className="pdfb-save-tooltip__label">{getLabel()}</div>
          
          {state === 'saving' && (
            <div className="pdfb-save-tooltip__progress-bar">
              <div
                className="pdfb-save-tooltip__progress-fill"
          ...
          {state === 'idle' && (
            <button
              className="pdfb-save-tooltip__button"
              onClick={onSaveNow}
